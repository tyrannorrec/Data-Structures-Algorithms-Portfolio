...
int stackFull() {
    ...
    if (listLength(myList) == FULLSTACK) {
        return 1;
    } // IF
    else {
        return 0;
...
int push(int item) {
    ...
    int pushSuccess = insertFront(myList, item);
    return pushSuccess;
} // FUNCTION `push`
...
int populateStack() {
    ...
    int populateSuccess = populateListFront(myList);
...
// THE FOLLOWING CODE WAS GIVEN IN THE STARTER CODE FILE
...
// ---------------------------------------------------------- 
//  function PRINTMENU prints the menu choices in the STACK ADT
// ---------------------------------------------------------- 
