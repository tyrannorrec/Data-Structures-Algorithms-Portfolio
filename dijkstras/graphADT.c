...
vertexNode_t* createVertexNode(int inData) {
    ...
    vertexNode_t* newVertexNode = (vertexNode_t*) malloc(sizeof(vertexNode_t));
    ...
    newVertexNode->lastHop = -1;
    newVertexNode->totalCost = 9999;
...
} // FUNCTION `createEdgelist_t* makeList() {
...
void resetFlags() {
    ...
    for(int i = 1; i <= NUM_VERTICES; i++) {
        ...
        graphArray[i]->lastHop = -1;
        graphArray[i]->totalCost = 9999;
    } // FOR
...
/* --------------------------------------------------------------
 * `addToUnexploredArray()`
 * ------------------------------------------------------------ */
int addToUnexploredArray(int inData) {

    if (unexploredArraySize == MAX_VERTICES) {
        return -1;
    } // IF

    graphArray[inData]->visited = 1;
    unexploredArray[unexploredArraySize] = inData;
    unexploredArraySize++;
    return 1;

} // FUNCTION `addToUnexploredArray()` 

/* --------------------------------------------------------------
 * `removeFromUnexploredArray()`
 * ------------------------------------------------------------ */
void removeFromUnexploredArray(int inData) {

   int index;

   for (int i = 0; i < unexploredArraySize; i++) {
        if (unexploredArray[i] == inData) {
            index = i;
            break;
        } // IF
    } // FOR

    for (int j = index; j < unexploredArraySize; j++) {
        unexploredArray[j] = unexploredArray[j + 1];
    } // FOR

    unexploredArraySize--;

} // FUNCTION `removeFromUnexploredArray()`
...
int findMinCostVertex() {
    ...
    int minCostVertexData = 0;
    int minCost = 9999;
    int currData;

    for (int i = 0; i < unexploredArraySize; i++) {
        currData = unexploredArray[i];
        if (graphArray[currData]->totalCost < minCost) {
            minCost = graphArray[currData]->totalCost;
            minCostVertexData = currData;
        } // IF
    } // FOR

    return minCostVertexData;
...
void printDijkstraTable() {
    ...
    vertexNode_t* tempNode;

    printf("\nVERTEX          COST            LAST HOP\n"
           "----------------------------------------\n");

    for (int i = 1; i <= NUM_VERTICES; i++) {
	tempNode = graphArray[i];
        printf("%d\t\t%d\t\t%d\n",
                tempNode->data, tempNode->totalCost, tempNode->lastHop);
    } // FOR

    printf("\n");

} // FUNCTION `printDijkstraTable()`
...
int dijkstra(int source) {
    ...
    int potentialCost;
    int neighborIndex;
    vertexNode_t* neighbor;
    edgeNode_t* edgeNodeIterator;

    // SET SOURCE VERTEX ATTRIBUTES AND ADD TO UNEXPLORED ARRAY
    addToUnexploredArray(source);
    vertexNode_t* currVertex = graphArray[source];
    currVertex->lastHop = -1;
    currVertex->totalCost = 0;

    while(unexploredArraySize > 0) {

        // FOR ALL EDGES OF CURRENT VERTEX
        edgeNodeIterator = currVertex->edges->head;
        while (edgeNodeIterator != NULL) {

            // IF POTENTIAL COST OF PATH TO NEIGHBOR IS LESS THAN EXISTING PATH,
            // UPDATE TOTAL COST AND LAST HOP OF NEIGHBOR
            neighborIndex = edgeNodeIterator->data;
            neighbor = graphArray[neighborIndex];
            potentialCost = (currVertex->totalCost) + (edgeNodeIterator->edgeCost);

            if (potentialCost < (neighbor->totalCost)) {
                neighbor->totalCost = potentialCost;
                neighbor->lastHop = currVertex->data;
            } // IF

            // IF THE NEIGHBOR IS NOT IN UNEXPLORED ARRAY, ADD TO ARRAY
            if (isVisited(edgeNodeIterator->data) < 1) {
                addToUnexploredArray(edgeNodeIterator->data);
            } // IF

            edgeNodeIterator = edgeNodeIterator->next;
        } // FOR

        // REMOVE CURR VERTEX FROM UNEXPLORED ARR
        removeFromUnexploredArray(currVertex->data);

        // CHOOSE UNEXPLORED VERTEX WITH MIN TOTAL COST
        int minCostVertexData = findMinCostVertex();
        currVertex = graphArray[minCostVertexData];
...
int printShortestPaths(int source) {
    ...
    int destination, pathCost, poppedIndex;
    vertexNode_t* vertexIterator;
    myStackList = createList();
    ...
    for (int i = 1; i <= NUM_VERTICES; i++) {
        if (i != source) {
            destination = i;
	    vertexIterator = graphArray[destination];            
            pathCost = vertexIterator->totalCost;

            printf("Path from %d to %d with Cost %d:\t%d", 
		    source, destination, pathCost, source);

            while (vertexIterator->lastHop != -1) {
                push(vertexIterator->data);
                vertexIterator = graphArray[vertexIterator->lastHop];
	    } // WHILE

            while (!stackEmpty()) {
	        poppedIndex = pop();
                printf("  -->  %d", poppedIndex);
	    } // WHILE   
