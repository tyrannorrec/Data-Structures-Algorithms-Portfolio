...
int queueEmpty() {
    ...
    // If Front and Rear are both -1, queue is empty
    if (FRONT == -1 && REAR == -1) {
        return 1;
    } // IF
...
int queueFull() {
    ...
    // If Front is 0 and Rear is at last index, OR
    // if Front is 1 greater than Rear, then queue is full.
...
void printQueue() {
    ...
    // If queue is empty, print error message.
    if (queueEmpty()) {
        printf("The queue is empty. Nothing to print.\n");
    } // IF

    // Else if Front <= Rear, print all items from Front to Rear
    else if (FRONT <= REAR) {
        int currFront = FRONT;
    ...
    // Else if Rear < Front, print (Front -> Last Index) and (0 -> Rear)
    else {
        int currFront = FRONT;

        // While iterator does not surpass last index, print value.
        while (currFront <= MAXSIZE - 1) {
            printf("%d  ", queueArray[currFront]);
            currFront++;
        } // WHILE
